...
<template lang="pug">
.grid.w-full.grid-cols-4.overflow-y-scroll(
  v-if="scenario"
  class="@container"
  style="grid-template-rows: 16rem auto"
)
  .relative.col-span-4.border-b
    //- Cover image.
    ...
    Placeholder.h-full.w-full.bg-neutral-100(v-else title="No cover image")

    .absolute.bottom-0.right-0.flex.p-3
      button.btn-pressable.btn.btn-sm-square.btn-neutral.rounded.border(
  ...
  //- Side section.
  .-mt-48.flex.flex-col.gap-3.p-3
    ScenarioVue.cursor-pointer.rounded-lg.border-4.border-white.shadow-lg.transition-transform.pressable-sm(
      :scenario
      layout="grid"
      :narrow-padding="true"
      @click="$emit('back')"
    ...
    //- Attributes.
    .flex.flex-col.gap-2.rounded-lg.bg-white.p-3.text-sm.shadow-lg(
      style="grid-template-columns: auto auto"
    )
      p.col-span-2.italic.leading-tight {{ scenario.content.about }}
  ...
  //- Main section.
  .col-span-3.flex.w-full.flex-col.gap-2.py-3.pr-3(class="@container")
